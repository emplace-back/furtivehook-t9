...
bool ImGui::small_checkbox(const char* label, bool* v)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    RenderFrame(check_bb.Min + ImVec2(pad + 3, pad + 3), check_bb.Max - ImVec2(pad + 3, pad + 3), GetColorU32((held && hovered) ? ImGuiCol_FrameBgActive : hovered ? ImGuiCol_FrameBgHovered : ImGuiCol_FrameBg), true, style.FrameRounding);
    if (*v)
    {
        window->DrawList->AddRectFilled(check_bb.Min + ImVec2(pad + 3, pad + 3), check_bb.Max - ImVec2(pad + 3, pad + 3), GetColorU32(ImGuiCol_CheckMark), style.FrameRounding);
    }
...
bool ImGui::BeginCombo(const std::string& label, const std::string& preview_value, ImGuiComboFlags flags)
{
    ...
    if (preview_value.data() != NULL && !(flags & ImGuiComboFlags_NoPreview))
        RenderTextClipped(frame_bb.Min + style.FramePadding, ImVec2(value_x2, frame_bb.Max.y), preview_value.data(), NULL, NULL, ImVec2(0.0f, 0.0f));
    if (label_size.x > 0)
        RenderText(ImVec2(frame_bb.Max.x + style.ItemInnerSpacing.x, frame_bb.Min.y + style.FramePadding.y), label.data());
...
bool ImGui::InputTextA(const char* label, std::string* str, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* user_data)
{
...
bool ImGui::Selectable(const std::string& label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    RenderTextClipped(text_min, text_max, label.data(), NULL, &label_size, style.SelectableTextAlign, &bb_enlarged);
...
bool ImGui::ListBox(const std::string& label, const std::uint32_t* currIndex, std::vector<std::string>& values)
{
	const auto vector_getter = [](void* vec, int idx, const char** out_text)
	{
	...
	if (!values.empty())
	{
		return ImGui::ListBox(label.data(), reinterpret_cast<int*>(&currIndex), vector_getter, reinterpret_cast<void*>(&values), values.size());
	}
...
bool ImGui::BeginMenu(const std::string& label, bool enabled)
{
    ...
    if (window->DC.LayoutType == ImGuiLayoutType_Horizontal)
    {
        ...
        pressed = Selectable(label.data(), menu_is_open, ImGuiSelectableFlags_NoHoldingActiveID | ImGuiSelectableFlags_SelectOnClick | ImGuiSelectableFlags_DontClosePopups | (!enabled ? ImGuiSelectableFlags_Disabled : 0), ImVec2(w, 0.0f));
        PopStyleVar();
    ...
    else
    {
        ...
        pressed = Selectable(label.data(), menu_is_open, ImGuiSelectableFlags_NoHoldingActiveID | ImGuiSelectableFlags_SelectOnClick | ImGuiSelectableFlags_DontClosePopups | ImGuiSelectableFlags_SpanAvailWidth | (!enabled ? ImGuiSelectableFlags_Disabled : 0), ImVec2(min_w, 0.0f));
...
bool ImGui::MenuItem(const std::string& label, const char* shortcut, bool selected, bool enabled)
{
    ...
    else
    {
        ...
        float extra_w = ImMax(0.0f, GetContentRegionAvail().x - min_w);
        pressed = Selectable(label.data(), false, flags | ImGuiSelectableFlags_SpanAvailWidth, ImVec2(min_w, 0.0f));
        if (shortcut_w > 0.0f)
        {
